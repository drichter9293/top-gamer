...
const getResultsForTeam = (score: number, expectedValue: number, team: Team, playerRatings: PlayerRatings, gameID: number, placement: number): Result[] => {
  const teamEloSum = team.reduce((eloSum, playerID) => eloSum + playerRatings[playerID], 0);
...
export const getResultsForGame = (playerRatings: PlayerRatings, placements: GameResult['placements'], gameID: number): Result[] => {
  let results: Result[] = [];
  ...
  results = results.concat(getResultsForTeam(1, winningTeamExpectedValue, placements[0][0], playerRatings, gameID, 0));
  placements[1].forEach((team, teamIndex) => {
    const teamQ = losingTeamQs[teamIndex];
    const expectedValue = teamQ / qSum;
    results = results.concat(getResultsForTeam(0, expectedValue, team, playerRatings, gameID, 1));
  });
  return results;
}